      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          // Transform waypoints from global coordinate to vehicle coordinate
          ...
          // The polynomial is fitted to 3rd order.
          auto coeffs = polyfit(waypts_x, waypts_y, 3);

          // The cross track error is calculated by evaluating at polynomial at x, f(x)
          // and subtracting y.
          // After transform to vehicle coordinate x0 = y0 = 0;
          double cte = polyeval(coeffs, 0);
          // Due to the sign starting at 0, the orientation error is -f'(x).
          double epsi = -atan(coeffs[1]);

          // State in vehicle coordinate, px, py and psi are always 0.
