...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // The part of the state for error of cte, psi and reference velocity
    for(unsigned int t = 0; t < N; t++){
      fg[0] += weight_cte * CppAD::pow(vars[cte_start + t], 2);
      fg[0] += weight_epsi * CppAD::pow(vars[epsi_start + t], 2);
      fg[0] += weight_refv * CppAD::pow(vars[v_start + t] - ref_v, 2);
    }
    ...
    for(unsigned int t = 0; t < N-2; t++){
      fg[0] += weight_delta_gap * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += weight_a_gap * CppAD::pow(vars[a_start + t + 1] - vars[delta_start + t], 2);
    }
    ...
    for (unsigned int t = 1; t < N; t++) {
      ...
      // Assuming the reference line is a 2nd order polynomial.
